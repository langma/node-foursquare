...
module.exports = function(config) {
   ...
   */
  function likeTip(tipId, params, accessToken, callback) {
    logger.enter('likeTip');

    if(!tipId) {
      logger.error('likeTip: tipId is required.');
      callback(new Error('Tips.likeTip: tipId is required.'));
      return;
    }

    params = params || {};

    core.callApi(path.join('/tips', tipId, 'like'), accessToken, params, callback, 'POST');
  }
   ...
   * @param {String} accessToken The access token provided by Foursquare for the current user.
   * @param {Function} callback The function to call with results, function({Error} error, {Object} results).
   * @see https://developer.foursquare.com/docs/tips/flag
   */
  function flagTip(tipId, problem, params, accessToken, callback) {
    logger.enter('likeTip');
    ...
    core.callApi(path.join('/tips', tipId, 'flag'), accessToken, params, callback, 'POST');
  }
